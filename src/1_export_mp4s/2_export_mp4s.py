...
from video_processing.face_processing import get_roi_for_each_face2, resize_and_square_face
...
VIDEO_ROOT = os.path.join(PARENT_DIR, "data")  # This is the DFDC source video root, ie this folder should contain dfdc_train_part_0, dfdc_train_part_1 etc.
OUTPUT_MP4_DIR = os.path.join(PARENT_DIR, "data", "face_videos_by_part")  # Where we save our new MP4s
BOX_PICKLE_DIR = os.path.join(PARENT_DIR, "data", "rois")  # Where the pickle are kept containing the ROIs for the faces
...
def export_chunks_for_video(params):
    ...
    pickle_output_dir = os.path.join(OUTPUT_MP4_DIR, os.path.basename(os.path.dirname(mp4_path)))

    video_all, _ = load_video(mp4_path,
                           every_n_frames=1,
                           to_rgb=True,
                           rescale=None,
                           inc_pil=False,
    ...
    for i_chunk in range(len(video_all) // FRAME_CLUMP_SIZE_FRAMES):  # -> [0, 1]
        print(f"Chunk {i_chunk} of {len(video_all) // FRAME_CLUMP_SIZE_FRAMES}")
        ...
        faces = get_roi_for_each_face2(faces_by_frame=boxes,
        ...
        for i_face, face in enumerate(faces):
            ...
            print(f"Writing {os.path.join(pickle_output_dir, f'{mp4_name}_{i_chunk}_{i_face}.mp4')}")
